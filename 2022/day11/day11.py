...
def monkey_factory(lines):
    ...
    return Monkey(items, operation, divisible, targets)


def run(fname, n_rounds):
    monkeys = []
    ...
    while i < len(lines):
        ...
        monkey = monkey_factory(lines[i + 1 : i + 6])
...
if __name__ == "__main__":
    assert 10605 == run("sample.txt", 20)
    print(run("input.txt", 20))
